<div align="center">
  ...
  <h1>閃念文本</h1>
  <p><strong>適用於 uTools 的輕量文字工作台，用於快速記錄、編輯 Markdown、開啟不同編碼的檔案，以及製作分享圖片。</strong></p>
</div>

閃念文本是一個 uTools 插件，將純文字、Markdown 和程式碼編輯集中於同一個視窗，並提供歷史記錄、草稿恢復、文字排版、編碼識別、圖片分享和可選的 AI 功能。

## 打賞

如果閃念文本為您節省了臨時記錄、文字整理和圖片製作的時間，歡迎透過下方二維碼打賞作者。請選擇支持方式：

<p align="center">
  <img src="../public/donate/wechat-pay.png" width="220" alt="微信支付打賞二維碼">
  &nbsp;&nbsp;&nbsp;&nbsp;
  <img src="../public/donate/alipay.png" width="220" alt="支付寶打賞二維碼">
</p>

## 功能

- 純文字、Markdown 和程式碼模式；Markdown 標記範圍內顯示原始碼，移出後恢復渲染。
- 歷史條目、自動保存、草稿恢復、搜尋、預覽、重新命名、刪除和排序。
...
- 提供便簽、程式碼、小紅書、知乎、微信和 X 分享圖片工作台。
- 內置簡中、繁中、英文、日文、越南文、韓文、西文、法文、葡萄牙文、俄文、德文和意大利文介面。

## 安裝與開發

1. 安裝並啟動 [uTools](https://u.tools/)。
2. 從 [uTools 插件頁](https://www.u-tools.cn/plugins/detail/%E9%97%AA%E5%BF%B5%E6%96%87%E6%9C%AC/) 直接安裝，也可從 [GitHub Releases](https://github.com/realSilasYang/flash-note-text/releases) 下載插件或在本地構建。
3. 在 uTools 開發者工具中載入生成的 `dist` 目錄。
